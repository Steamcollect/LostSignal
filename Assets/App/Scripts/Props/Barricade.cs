...
public class Barricade : MonoBehaviour
{
    ...
    private void OnDrawGizmosSelected()
    {
        ...
        var radius = bounds.extents.magnitude + m_DistanceThreshold;
        ...
        for (int i = 0; i < samples; i++)
        {
            ...
            var pointOnSphere = center + new Vector3(x, y, z) * radius;
            var closestPoint = m_TriggerCollider.ClosestPoint(pointOnSphere);
            var direction = (pointOnSphere - closestPoint).normalized;
            
            if (direction != Vector3.zero)
            {
                Handles.DrawSolidDisc(closestPoint + direction * m_DistanceThreshold, Camera.current.transform.forward, 0.05f);
            }
        }
        #endif
    }
